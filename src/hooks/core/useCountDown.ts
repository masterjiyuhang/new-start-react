...
function useCountDown(options: ITime, cb?: () => void, noImmediate?: boolean): ICbTime {
  ...
  const [diffTime, setDiffTime] = useState(0) /** 组件接收到参数时的时间 */
  const entryTime = useRef<number>(0) /** 当前倒计时要求的时间差 */
  const maxTime = useRef<number>(0) /** 是否可以执行回调 */
  ...
  useEffect(() => {
    ...
    if ((currentTime > 0 && endTime > 0) || differTime > 0) {
      ...
      if (maxTime.current <= 0 && (noImmediate ?? false)) {
