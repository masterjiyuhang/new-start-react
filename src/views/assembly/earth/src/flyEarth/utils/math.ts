...
import { type Coordinates3D } from "@/views/assembly/earth/src/flyEarth/interface";
 ...
 */
const lon2xyz = (R: number, longitude: number, latitude: number): Coordinates3D => {
...
const _3Dto2D = (start: Vector3, end: Vector3) => {
	...
	const xoyQuaternion = new Quaternion().setFromUnitVectors(normal, new Vector3(0, 0, 1));
	const startXoy = start.clone().applyQuaternion(xoyQuaternion);
	const endXoy = end.clone().applyQuaternion(xoyQuaternion);

	const middleXoy = new Vector3().addVectors(startXoy, endXoy).multiplyScalar(0.5).normalize();
	const xoyQuaternionMiddle = new Quaternion().setFromUnitVectors(middleXoy, new Vector3(0, 1, 0));
	const startXoyMiddle = startXoy.clone().applyQuaternion(xoyQuaternionMiddle);
	const endXoyMiddle = endXoy.clone().applyQuaternion(xoyQuaternionMiddle);
	// scene.add(helpView(8,startXoyMiddle,endXoyMiddle))

	const quaternionInverse = xoyQuaternion.clone().invert().multiply(xoyQuaternionMiddle.clone().invert());
	return {
		// 返回两次旋转四元数的逆四元数
		quaternion: quaternionInverse,
		// 范围两次旋转后在XOY平面上关于y轴对称的圆弧起点和结束点坐标
		startPoint3D: startXoyMiddle,
		endPoint3D: endXoyMiddle
	};
};
...
function getFunctionExpression(src: Vector3, dist: Vector3) {
	...
	const y1 = (dist.y + src.y) / 2;
	const bPerpendicular = x1 - kPerpendicular * y1;
	const distance = src.distanceTo(dist);
	return getPointByDistance(x1, y1, kPerpendicular, bPerpendicular, distance / 5);
}
// 根据已知函数距离求点位
